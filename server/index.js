...
// Custom CORS middleware to ensure single headers
// Using res.header() instead of res.setHeader() to prevent duplicate headers
// as res.header() will overwrite existing headers with the same name.
app.use((req, res, next) => {
    const origin = req.headers.origin;
    ...
    if (allowedOrigins.includes(origin)) {
        ...
        res.header('Access-Control-Allow-Credentials', 'true'); // Changed to .header()
...
app.use((err, req, res, next) => {
    console.error('Global error handler caught:', err);
    // Ensure headers haven't been sent before attempting to set status/json
    if (!res.headersSent) {
        res.status(500).json({
            success: false,
            message: 'Internal server error',
        ...
        console.error('⚠️ Headers already sent when global error handler was invoked.');
    }
});
