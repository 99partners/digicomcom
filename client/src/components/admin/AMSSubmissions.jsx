...
const AMSSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:5050/api/platform-ams/submissions', {
        credentials: 'include'
      });
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5050/api/platform-ams/submissions/${id}/status`, {
        method: 'PATCH',
        headers: {
