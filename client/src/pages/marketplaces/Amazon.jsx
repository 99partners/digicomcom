...
const Amazon = () => {
  ...
  const seo = t('marketplaces.amazon.seo', { returnObjects: true });
  const header = t('marketplaces.amazon.header', { returnObjects: true });
  const overview = t('marketplaces.amazon.overview', { returnObjects: true });
  const requirements = t('marketplaces.amazon.requirements', { returnObjects: true });
  const steps = t('marketplaces.amazon.steps', { returnObjects: true });
  const commonQuestions = t('marketplaces.amazon.commonQuestions.list', { returnObjects: true });
  const faqCategories = t('marketplaces.amazon.faq.categories', { returnObjects: true });
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href="https://99digicom.com/partners/marketplaces/amazon" />
        
        <meta property="og:type" content="website" />
        <meta property="og:title" content={seo.title} />
        <meta property="og:description" content={seo.description} />
        <meta property="og:image" content="https://99digicom.com/og-image.jpg" />
        <meta property="og:url" content="https://99digicom.com/partners/marketplaces/amazon" />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          url: "https://99digicom.com/partners/marketplaces/amazon"
        })}</script>
      </Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col items-center justify-center mb-10">
            <div className="bg-white p-6 rounded-xl shadow-md flex flex-col items-center">
              <img src={AmazonLogo} alt="Amazon Logo" className="w-48 h-48 object-contain mb-4" loading="lazy" />
                ...
                {requirements.items.map((item, idx) => (
                  <div key={idx} className="flex items-center bg-white p-3 rounded-lg shadow-sm">
                    <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center mr-3">
                      {idx === 0 && <User className="h-5 w-5 text-green-600" />}
                      {idx === 1 && <CreditCard className="h-5 w-5 text-green-600" />}
                      {idx === 2 && <Shield className="h-5 w-5 text-green-600" />}
                      {idx === 3 && <Building className="h-5 w-5 text-green-600" />}
                      {idx === 4 && <Package className="h-5 w-5 text-green-600" />}
                      {idx === 5 && <span className="font-bold text-green-600">18+</span>}
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {steps.list.map((step, index) => (
                ...
                >
                  ...
                  <div>
                    <h3 className="font-semibold text-lg text-gray-900 mb-1">{step}</h3>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="mb-12">
            <div className="bg-white rounded-lg shadow-md p-8 border-t-4 border-green-500">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-green-700">{t('marketplaces.amazon.commonQuestions.title')}</h2>
              </div>
          ...
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-center text-green-700 mb-8">{t('marketplaces.amazon.faq.title')}</h2>
